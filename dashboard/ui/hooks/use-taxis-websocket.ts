...
const WS_API_URL = "ws://localhost:8000/ws/taxis";
const PING_INTERVAL_MS = 15000;
const MAX_RECONNECT_DELAY = 30000;
const INITIAL_RECONNECT_DELAY = 1000;

export function useTaxisWebSocket(wsEnabled: boolean) {
    ...
    function connect() {
      ...
      wsRef.current.onmessage = (event) => {
        try {
          ...
          if (data.taxis) {
            taxiBatches.push(...data.taxis);
            expectedBatches = data.total_batches;
            if (data.batch_num === expectedBatches) {
              setTaxis(taxiBatches);
              taxiBatches = [];
            }
          }
        } catch (err) {
