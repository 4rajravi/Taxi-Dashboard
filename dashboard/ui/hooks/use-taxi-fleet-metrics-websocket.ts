...
const WS_API_URL = "ws://localhost:8000/ws/taxi_fleet_metrics";
const PING_INTERVAL_MS = 15000;
const MAX_RECONNECT_DELAY = 30000;
const INITIAL_RECONNECT_DELAY = 1000;

export function useTaxiFleetMetricsWebSocket(wsEnabled: boolean) {
  // Get actions from the taxi data store
  const setTaxiFleetMetrics = useTaxiFleetMetricsStore((state) => state.setTaxiFleetMetrics);
    ...
    function connect() {
      ...
      wsRef.current.onmessage = (event) => {
        try {
          ...
          setTaxiFleetMetrics(data.taxi_fleet_metrics);
        } catch (err) {
