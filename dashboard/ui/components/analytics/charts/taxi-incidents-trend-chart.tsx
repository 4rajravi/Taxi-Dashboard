...
export const description =
  "An interactive stacked bar chart with legend for taxi incidents trend";
...
export default function TaxiIncidentsTrendChart({
  taxiId,
  taxiIncidentTrendChartData,
}: {
  taxiId: number;
  taxiIncidentTrendChartData: Array<{
    time: string;
    ...
    <Card className="py-4">
      <CardHeader className="flex items-center gap-2 space-y-0 border-b py-5 sm:flex-row">
        <div className="grid flex-1 gap-1">
          ...
          <CardDescription className="text-xs">
            Taxi <span className="font-semibold text-slate-900">{taxiId}</span>{" "}
      ...
      <CardContent>
        <ChartContainer config={chartConfig}>
          <BarChart
            accessibilityLayer
            data={taxiIncidentTrendChartData}
            barCategoryGap="20%"
            maxBarSize={40}
            layout="horizontal"
            width={40}
