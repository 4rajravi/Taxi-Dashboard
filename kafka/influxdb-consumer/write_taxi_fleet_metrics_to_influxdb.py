...
from helpers.influxdb_taxi_fleet_metrics_writer import InfluxDBTaxiFleetMetricsWriter
...
from config import (
    ...
    KAFKA_INFLUXDB_TAXI_FLEET_METRICS_CONSUMER_TOPIC,
    KAFKA_INFLUXDB_TAXI_FLEET_METRICS_CONSUMER_GROUP,
...
class KafkaInfluxDBTaxiFleetMetricsConsumer:
    def __init__(self, logger=None):
        self.logger = logger or get_logger(
            "KafkaInfluxDBTaxiFleetMetricsConsumer", "influxdb_taxi_fleet_metrics_consumer.log"
        )
        self.consumer = None
        self.influxdb_taxi_fleet_metrics_writer = InfluxDBTaxiFleetMetricsWriter(logger=logger)
    ...
    async def start_kafka_influxdb_taxi_fleet_metrics_consumer(
        self, retries: int = 5, delay: int = 1
    ):
        await self.wait_until_topic_available(
            KAFKA_INFLUXDB_TAXI_FLEET_METRICS_CONSUMER_TOPIC, retries, delay
        )
        ...
        for attempt in range(1, retries + 1):
            self.consumer = AIOKafkaConsumer(
                ...
                group_id=KAFKA_INFLUXDB_TAXI_FLEET_METRICS_CONSUMER_GROUP,
            ...
            try:
                ...
                if not partitions:
                    raise RuntimeError(
                        f"No partitions found for topic {KAFKA_INFLUXDB_TAXI_FLEET_METRICS_CONSUMER_TOPIC}"
                    )
    ...
    async def consume_taxi_fleet_metrics_messages(self):
        try:
            batch = []
            batch_size = 500  # Optimal batch size depends on system performance and throughput
            ...
            try:
                async for msg in self.consumer:
                    ...
                    if len(batch) >= batch_size or (current_time - last_write_time) >= batch_write_interval:
                        try:
                            self.influxdb_taxi_fleet_metrics_writer.write_fleet_metrics_to_db(batch)
                            self.logger.info(f"[InfluxDB Write] Batch of {len(batch)} fleet_metrics")
                            batch.clear()
                ...
                if batch:
                    try:
                        self.influxdb_taxi_fleet_metrics_writer.write_fleet_metrics_to_db(batch)
                        self.logger.info(f"[InfluxDB Write] Final batch of {len(batch)} fleet_metrics")
                    except Exception as e:
    ...
    async def start(self):
        try:
            await self.start_kafka_influxdb_taxi_fleet_metrics_consumer()
            await self.consume_taxi_fleet_metrics_messages()
        except Exception:
            self.logger.critical(
                "Fatal error during KafkaInfluxDBTaxiFleetMetricsConsumer startup",
                exc_info=True,
            )


async def run_kafka_influxdb_taxi_fleet_metrics_consumer():
    try:
        logger = get_logger(
            "KafkaInfluxDBTaxiFleetMetricsConsumer", "influxdb_taxi_fleet_metrics_consumer.log"
        )
        ...
        consumer = KafkaInfluxDBTaxiFleetMetricsConsumer(logger=logger)
