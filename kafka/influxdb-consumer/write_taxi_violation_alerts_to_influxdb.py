...
from helpers.influxdb_taxi_violation_alerts_writer import InfluxDBTaxiViolationAlertsWriter
...
from config import (
    ...
    KAFKA_INFLUXDB_TAXI_VIOLATION_ALERTS_CONSUMER_TOPIC,
    KAFKA_INFLUXDB_TAXI_VIOLATION_ALERTS_CONSUMER_GROUP,
...
class KafkaInfluxDBTaxiViolationAlertsConsumer:
    def __init__(self, logger=None):
        self.logger = logger or get_logger(
            "KafkaInfluxDBTaxiViolationAlertsConsumer", "influxdb_taxi_violation_alerts_consumer.log"
        )
        self.consumer = None
        self.influxdb_taxi_violation_alerts_writer = InfluxDBTaxiViolationAlertsWriter(logger=logger)
    ...
    async def start_kafka_influxdb_taxi_violation_alerts_consumer(
        self, retries: int = 5, delay: int = 1
    ):
        await self.wait_until_topic_available(
            KAFKA_INFLUXDB_TAXI_VIOLATION_ALERTS_CONSUMER_TOPIC, retries, delay
        )
        ...
        for attempt in range(1, retries + 1):
            self.consumer = AIOKafkaConsumer(
                ...
                group_id=KAFKA_INFLUXDB_TAXI_VIOLATION_ALERTS_CONSUMER_GROUP,
            ...
            try:
                ...
                if not partitions:
                    raise RuntimeError(
                        f"No partitions found for topic {KAFKA_INFLUXDB_TAXI_VIOLATION_ALERTS_CONSUMER_TOPIC}"
                    )
    ...
    async def consume_taxi_violation_alerts_messages(self):
        try:
            batch = []
            ...
            try:
                async for msg in self.consumer:
                    ...
                    if len(batch) >= batch_size or (current_time - last_write_time) >= batch_write_interval:
                        try:
                            self.influxdb_taxi_violation_alerts_writer.write_taxi_violation_alerts_to_db(batch)
                            self.logger.info(f"[InfluxDB Write] Batch of {len(batch)} violation_alerts")
                            batch.clear()
                ...
                if batch:
                    try:
                        self.influxdb_taxi_violation_alerts_writer.write_taxi_violation_alerts_to_db(batch)
                        self.logger.info(f"[InfluxDB Write] Final batch of {len(batch)} violation_alerts")
                    except Exception as e:
    ...
    async def start(self):
        try:
            await self.start_kafka_influxdb_taxi_violation_alerts_consumer()
            await self.consume_taxi_violation_alerts_messages()
        except Exception:
            self.logger.critical(
                "Fatal error during KafkaInfluxDBTaxiVioationAlertsConsumer startup",
                exc_info=True,
            )


async def run_kafka_influxdb_taxi_violation_alerts_consumer():
    try:
        logger = get_logger(
            "KafkaInfluxDBTaxiViolationAlertsConsumer", "influxdb_taxi_violation_alerts_consumer.log"
        )
        ...
        consumer = KafkaInfluxDBTaxiViolationAlertsConsumer(logger=logger)
        await consumer.start()

    except Exception:
        logger.critical(
            "Fatal error: influxdb taxi violation alerts consumer application could not start",
            exc_info=True,
        )
        sys.exit(1)
